    ...
    @org_data = {}
     ...
     if !session[:organization].blank?
        sql="SELECT contact_name, email, telephone, organization_name, organization_country, 
         organization_type_id, organization_guid, organization_web, api_key, package_name FROM organizations WHERE cartodb_id = #{session[:organization][:cartodb_id]}"
  ...
  def signup_validation
    ...
    if !session[:organization].blank?
        ...
        if @errors.count==0
          ...
          sql="UPDATE  organizations SET organization_guid ='#{params[:organization_guid]}', 
          password = md5('#{params[:password]}'), contact_name= '#{params[:contact_name]}', telephone = '#{params[:telephone]}', 
          organization_name = '#{params[:organization_name]}', organization_type_id ='#{params[:organization_type_id]}',  
          organization_country = '#{params[:organization_country]}', organization_web ='#{params[:organization_web]}', 
          api_key = '#{params[:api_key]}', package_name = '#{params[:package_name]}'
          where cartodb_id = #{session[:organization][:cartodb_id]}"
